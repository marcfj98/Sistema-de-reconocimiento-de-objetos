...
void ClasificarImagenes(Ptr<DTrees> arbol, Ptr<TrainData>& test_data, Mat* result, string* csv_loc);


int main(int argv, char** argc)
{

	// creacion del arbol de decisiones y sus parametros
	Ptr<TrainData> datos_entr = TrainData::loadFromCSV("datos_entrenamiento.csv", 1, 0, 1);
	Ptr<DTrees> arbol = DTrees::create();

	arbol->setCVFolds(1);
	arbol->setMaxDepth(5);

	arbol->train(datos_entr);
	cout << "Entrenamiento del arbol de decisiones completado\n\n" << endl;
	arbol->save("arbol_de_decision.xml");
	...
	ClasificarImagenes(arbol, test_data, result, csv_loc);
