...
void ClasificarImagenes(Ptr<SVM> svm, Ptr<TrainData>& test_data, Mat* result, string* csv_loc);

int main(int argv, char** argc)
{ 
	// ajuste de parametros y entrenamiento del clasificador SVM
	Ptr<TrainData> datos_entr = TrainData::loadFromCSV("datos_entrenamiento.csv", 1, 0, 1);
	Ptr<SVM> svm = SVM::create();
	svm->setType(SVM::C_SVC); // tipo de clasificador SVM (con soporte para coeficiente de regularizacion)
 	svm->setKernel(SVM::INTER); // kernel de interseccion del histograma. define el tipo de mapeado que se hace al espacio de caracteristicas
	svm->setC(0.1); // Coeficiente de regularizacion. Decide como se trata la diferencia del error al margen. Cuanto mayor, mas se castiga el error.
	svm->train(datos_entr);
	cout << "Entrenamiento del clasificador SVM completado\n\n" << endl;
	svm->save("clasificador_svm.xml");
	...
	ClasificarImagenes(svm, test_data, result, csv_loc);
